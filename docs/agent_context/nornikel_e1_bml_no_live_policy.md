...
PG storage foundation для E1 уже в `dev` (orchestrator `0008`, notification `0002` — см. `nornikel_e1_db_storage_ownership.md`). `/documents/{document_id}` и `/review/*` остаются контрактными skeleton routes без workflow wiring: endpoints возвращают `document_delete_not_implemented` или `review_storage_not_implemented` до E3 Backend/ML.
