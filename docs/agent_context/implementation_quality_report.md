...
3. **Doc drift** — `audit_report.md` и `domains/retrieval.md` описывают Neo4j/Qdrant как `planned`, хотя в коде они live.
...
| **retrieval** | 8005 | 4 | 14 | `app/qdrant_adapter.py`, `api/query.py` | Legacy `api/indexing.py` не смонтирован; doc drift |
...
| Query IR + hybrid search | ✅ | model + retrieval |
...
| 10 | Синхронизировать docs — audit_report, domains/retrieval | S |
...
|----------|-------------|-------------|
| `audit_report.md` | Neo4j, Qdrant — `planned` | Live adapters |
| `audit_report.md` | P1-02 UploadPage placeholder | UploadPage вызывает real API |
| `audit_report.md` | P1-01 UI без JWT | Login + JWT есть; RoleSwitcher остаётся |
| `domains/retrieval.md` | Qdrant `mode=mock` | `mode=live` в `qdrant_adapter.py` |
