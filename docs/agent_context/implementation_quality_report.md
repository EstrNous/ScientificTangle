...
| Export / notification | **3.5** | HTTP-сервисы wired: export artifacts через MinIO, notification API/events; остаются internal auth, PDF и runtime delivery gaps |
...
**MVP pipeline реализован** и превосходит типичный хакатон-MVP по архитектурной дисциплине: microservices, единые контракты, evidence-first, реальные граф и векторное хранилище. Главные разрывы post-MVP: **internal service auth для export/notification**, **mock-зависимости в UI для source refs**, **отсутствие зафиксированного live eval artifact**, **PDF export и notification post-ingestion delivery**.
...
1. **Export/notification wired, но не закрыты production-gaps** — internal endpoints без service-to-service auth; PDF export и post-ingestion notification delivery остаются backlog.
2. **UI зависит от mock catalog** — 10 компонентов импортируют `ui/src/api/mock/` для source refs даже в real-режиме.
3. **Doc drift** — текущий agent context обновлён (2026-07-04); поддерживать при изменениях
...
| Object storage | MinIO | Buckets `source-files`, `exports` | Ingestion + export artifacts |
| Cache / queue | Redis 7 | — | Подключён в compose |
| RDBMS | PostgreSQL 16 | Service schemas | auth_audit, orchestrator, chat_ui, notification; export runtime state в Redis/MinIO, authoritative metadata в orchestrator_db |
...
flowchart TB
  ...
  subgraph product_events [Export_Notifications]
    Export[export_8007]
...
| **orchestrator** | 8002 | 4 | 14 | `app/service/service.py` (940 строк); `api/ingestion.py`, `query.py` | God-service; internal export call без service auth |
...
| **retrieval** | 8005 | 4 | 14 | `app/qdrant_adapter.py`, `api/query.py` | Нужен seeded/live quality report; legacy `api/indexing.py` не смонтирован |
| **model** | 8006 | 5 | 31 | `app/api/v1.py` (13 endpoints), `services.py` | Live eval artifact не pinned |
| **export** | 8007 | 3.5 | 5 | `app/api/jobs.py`, `service/service.py`, `service/storage.py` | Internal auth; PDF |
| **notification** | 8008 | 3.5 | 6 | `app/api/notifications.py`, `interests.py`, `events.py` | Internal auth; post-ingestion delivery |
...
| Doc drift | `audit_report.md`, `domains/*.md`, сводные отчёты | Поддерживать при изменениях сервисов |
...
| Query IR + hybrid search | ⚠️ | dense + lexical + table + graph fusion wired; нужен seeded/live quality proof |
...
| Export MD/JSON/JSON-LD | ✅ wired via orchestrator/export | backend export includes evidence/sources/graph/gaps/conflicts/QueryIR/retrieval_trace; export service renders and stores artifact in MinIO |
...
| # | Задача | Размер |
|---|--------|--------|
| 1 | Service-to-service auth для export `POST /v1/jobs` и notification internal endpoints | M |
| 2 | Notification post-ingestion runtime delivery | M |
...
|----------|-------------|-------------|
| `domains/export.md` | `gateway → orchestrator → export → MinIO`; gaps: internal auth, PDF | Актуально |
| `domains/notification.md` | `wired`; gaps: internal auth, post-ingestion delivery | Актуально |
...
flowchart TB
  ...
  subgraph wired_gaps [Wired_with_gaps]
    Export[export]
    Notif[notification]
  end
  UI --> GW
