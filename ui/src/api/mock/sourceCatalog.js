const SOURCE_ENTRIES = {
  'span-1': {
    ...
    pages: {
      1: {
        raw_text:
          'Технический отчёт по режимам электроэкстракции никеля из сульфатных электролитов. Документ подготовлен для сравнения промышленных схем циркуляции католита и оценки влияния гидродинамики на выход по металлу.',
      },
      2: {
        raw_text:
          'Введение. Электроэкстракция никеля остаётся ключевой стадией гидрометаллургических схем. Стабильность процесса определяется составом раствора, температурой, плотностью тока и организацией движения электролита в ячейках.',
      },
      3: {
        raw_text:
          'Объектом анализа выступают установки с раздельными анодными и катодными камерами. Особое внимание уделено перепаду давления на мембранах, скорости оборота раствора и равномерности осаждения на катодах.',
      },
      4: {
        raw_text:
          'Глава 1. Исходные данные. Использовались промышленные замеры за 2022–2023 гг.: температура электролита 55–60 °C, содержание Ni²⁺ 55–65 г/л, pH 3,8–4,2. Концентрация примесей Fe и Co контролировалась еженедельно.',
      },
      5: {
        raw_text:
          'Глава 2. Оборудование. Схема включает накопительный бак, насосную группу, теплообменник и блок электролизёров с межэлектродным расстоянием 38 мм. Расход электролита задаётся частотным приводом насоса.',
      },
      6: {
        raw_text:
          'Глава 3. Методика испытаний. Для каждого режима фиксировали ток, напряжение на ячейке, температуру на входе и выходе, а также визуальное качество катодного осадка. Продолжительность одного цикла — 72 ч.',
      },
      7: {
        raw_text:
          'Результаты предварительных прогонов показали, что при снижении скорости потока ниже 1,5 м/ч увеличивается неоднородность толщины осадка и растёт доля блестящих участков с дефектами кристаллизации.',
      },
      8: {
        raw_text:
          'Наиболее устойчивый режим достигнут при активной циркуляции католита и поддержании температуры в узком коридоре. Отклонения по составу раствора компенсировались корректировкой кислотности.',
      },
      9: {
        raw_text:
          'Глава 4. Гидродинамика электролитных ячеек. Перед выбором режима циркуляции оценивают геометрию анодных камер, гидравлическое сопротивление линии и допустимый перепад давления на мембране.',
      },
      10: {
        raw_text:
          'Сравнение схем с однонаправленным и рециркуляционным движением электролита показало преимущество последней при работе с высокими токовыми нагрузками и повышенной концентрацией металла.',
      },
      11: {
        raw_text:
          'Глава 4 (продолжение). Перед выбором режима циркуляции оценивают геометрию анодных камер и допустимый перепад давления на мембране. Рекомендуется избегать зон застоя в нижней части катодной камеры.',
      },
      12: {
        raw_text:
  ...
  'span-2': {
    ...
    pages: {
      1: {
        raw_text:
          'Обзор отечественной практики обессоливания шахтных и промышленных сточных вод. Документ обобщает схемы подготовки, мембранную доочистку и типовые показатели качества на выходе.',
      },
      2: {
        raw_text:
          'Введение. Высокая минерализация исходной воды ограничивает прямое использование оборотных схем. Для горнодобывающих предприятий критичны устойчивость к колебаниям состава и предсказуемость эксплуатационных затрат.',
      },
      3: {
        raw_text:
          'Для шахтных вод с повышенной минерализацией рассматривают схемы с предварительным осветлением и мембранной доочисткой. На первом этапе удаляют взвешенные частицы и органику.',
      },
      4: {
        raw_text:
      ...
      5: {
        raw_text:
          'Экономическая эффективность схемы зависит от стоимости реагентов коагуляции и частоты замены мембранных модулей. Для пилотных установок рекомендуется вести журнал дифференциального давления на модулях.',
      },
      6: {
        raw_text:
          'Заключение. Для вод с TDS выше 1000 мг/дм³ целесообразна комбинированная схема: осветление, ультрафильтрация и обратный осмос с периодической химической промывкой.',
      },
    },
  },
  'span-3': {
    ...
    pages: {
      1: {
        raw_text:
          'Journal of Hydrometallurgy, 2024. Special issue on nickel electrowinning from sulfate electrolytes. Editorial summary of industrial trends and research priorities.',
      },
      2: {
        raw_text:
          'Abstract. This paper reviews operating windows for current density, electrolyte temperature and catholyte circulation in modern tank houses.',
      },
      3: {
        raw_text:
          'Introduction. Nickel electrowinning performance depends on electrolyte purity, anode quality and hydrodynamic conditions in the cathode compartment.',
      },
      4: {
        raw_text:
          'Materials and methods. Pilot cells were operated at 55 °C with synthetic electrolyte matching industrial composition. Flow rates were varied between 1.2 and 4.5 m/h.',
      },
      5: {
        raw_text:
          'Results (preliminary). Lower circulation rates increased localized concentration gradients and reduced current efficiency at the same applied current.',
      },
      6: {
        raw_text:
      ...
      8: {
        raw_text:
          'Авторы отмечают чувствительность процесса к содержанию примесей железа и кобальта в растворе. Регулярный анализ примесей снижает риск нестабильного осаждения.',
      },
      9: {
        raw_text:
          'Conclusion. Active catholyte circulation remains essential for high-current operation. Further work should quantify membrane pressure drop under fouling conditions.',
      },
    },
  },
  'span-4': {
    ...
    pages: {
      1: {
        raw_text:
          'Внутренний отчёт по результатам пилотных испытаний на установке №2. Распространение ограничено подразделениями технологической службы.',
      },
      2: {
        raw_text:
          'Отчёт подготовлен по результатам пилотных испытаний на установке №2. Сравнивались режимы с постоянной и переменной скоростью потока католита.',
      },
      3: {
        raw_text:
      ...
      4: {
        raw_text:
          'Рекомендуется еженедельный контроль содержания хлоридов и сульфатов в оборотном растворе. Превышение пороговых значений требует частичной смены электролита.',
      },
      5: {
        raw_text:
          'Приложение А. Сводная таблица режимов и показателей выхода по металлу за отчётный период. Наиболее стабильный результат получен в режиме B-2.',
      },
    },
  },
  'span-5': {
...
export function getDocumentViewPages(entry) {
  if (!entry) return [];
  const pageNumbers = getSourcePageNumbers(entry);
