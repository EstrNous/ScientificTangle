...
async def upload_dictionary(
    ...
    try:
        return await service.upload_dictionary(package, authorization, request.state.request_id)
...
async def list_dictionaries(
    ...
    try:
        return await service.list_dictionaries(authorization, request.state.request_id)
    except GatewayServiceError as error:
        raise_service_error(error)
...
async def get_active_dictionary(
    ...
    try:
        return await service.get_active_dictionary(authorization, request.state.request_id)
    except GatewayServiceError as error:
        raise_service_error(error)
...
async def activate_dictionary(
    ...
    try:
        return await service.activate_dictionary(version_id, authorization, request.state.request_id)
    except GatewayServiceError as error:
        raise_service_error(error)
