...
`auth_service` владеет учётными записями пользователей, регистрацией, профилями, паролями, JWT access-токенами, refresh-сессиями и базовыми ролями. Политики доступа к документам применяются gateway и доменными сервисами к доверенной идентичности из JWT.
